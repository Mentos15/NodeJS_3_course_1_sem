...
db.on('COMMIT', ()=>{db.active=true; db.commit();});
db.on('STAT', (res)=>{
  ...
  switch(oper[0]){
    ...
    if(param){
      ...
      if(param){
        ...
        db.comm=0;
        db.count=0;
        db.startStatisticTime = new Date();
        var interval = setInterval(()=>{
            console.log('...statistics calculation...')}, 2000);
        interval.unref();
        statId = setTimeout(()=>{
            db.stopStatisticTime = new Date();
            console.log(db.startStatisticTime);
            console.log(`calculation stopped: ${db.stopStatisticTime - db.startStatisticTime}`);
            clearInterval(interval);  
            db.active = false; rl.prompt();}, param*1000);
            statId.unref();
      }else{        
        clearTimeout(statId);
        statId=0;
