...
let writeHTTP405=(res)=>{
	res.statusCode = 405;
	res.statusMessage = 'Используйте доругой метод';
	res.end('Используйте доругой метод');
}
	...
	if(req.method=='GET'){
        ...
        fs.readFile('Studentlist.json',(e,data)=>
        {
          if(e) console.log('Ошибка: ',e);
              ...
              os.forEach(element => {
                  if(element.id==x)
                  {
                      result={id:element.id, name:element.name,birth:element.birth, speciality:element.speciality}; 
                      console.log(result); 
                  }
              });
              if(result!='')
      ...
      req.on('end',()=>{
          ...
          fs.readFile('./Studentlist.json',(e,data)=>
          {
            ...
            os.forEach(element => {
              result2+=`{"id":${element.id}, "name":"${element.name}","birth":"${element.birth}","speciality":"${element.speciality}"},`;
      ...
      if(url.parse(req.url).pathname=== '/')
        ...
        fs.readFile('./Studentlist.json',(e,data)=>
        {
            if(e) console.log('Ошибка: ',e);
              ...
              else result +=`{"id":${element.id}, "name":"${element.name}","birth":"${element.birth}","speciality":"${element.speciality}"},`;
                                  });
            }
            if(k==1)
            {
        ...
        fs.readFile('./Studentlist.json',(e,data)=>
          {
            if(e) console.log('Ошибка: ',e);
                ...
                if(x==element.id)
                {
                  body +=`{"id":${element.id}, "name":"${element.name}","birth":"${element.birth}","speciality":"${element.speciality}"},`;
                  k=1;
                  }
                else result +=`{"id":${element.id}, "name":"${element.name}","birth":"${element.birth}","speciality":"${element.speciality}"},`;
                                    });
